...
def process_mseed_file(mseed_file, catalog_file):
    ...
    event_id = os.path.basename(mseed_file).split('_')[-1].split('.')[0]  # Extract event ID
    event_id = event_id.replace("evid", "evid")  # Maintain 'evid' prefix

    # Load event time data from CSV
    event_time_data = pd.read_csv(catalog_file)
    ...
    # Plotting
    plt.figure(figsize=(10, 16))  # Increased figure size to accommodate 7 plots

    # Plot Time vs Energy (from rate of energy change)
    plt.subplot(7, 1, 1)
    ...
    plt.xlabel('')
    plt.ylabel('Energy (J)')
    plt.title(f'{os.path.basename(mseed_file)} - Time vs Energy')
    plt.grid(True)
    plt.legend()
...
if __name__ == "__main__":
    # Example usage: pass the mseed file and CSV catalog
    mseed_file = '/home/ayden/nasa/space_apps_2024_seismic_detection/data/lunar/training/data/S12_GradeA/xa.s12.00.mhz.1971-05-13HR00_evid00033.mseed'  # Replace with your file
    catalog_file = '/home/ayden/nasa/space_apps_2024_seismic_detection/data/lunar/training/catalogs/apollo12_catalog_GradeA_final.csv'
    
    process_mseed_file(mseed_file, catalog_file)
